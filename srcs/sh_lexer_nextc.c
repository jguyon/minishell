...
/*   Updated: 2017/04/05 18:42:16 by jguyon           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	nextc_normal(t_sh_lexer *lex)
{
	...
	{
		lex->curr_ctx = lex->curr_char == '"' ? SH_CTX_DQUOTE : SH_CTX_SQUOTE;
		sh_lexer_nextc(lex);
	}
...
static void	nextc_dquote(t_sh_lexer *lex)
{
	if (lex->curr_char == '"')
	{
		lex->curr_ctx = SH_CTX_NORMAL;
...
void		sh_lexer_nextc(t_sh_lexer *lex)
{
	...
	if (lex->curr_char == FT_EOF)
		...
		nextc_squote(lex);
	else if (lex->curr_ctx == SH_CTX_DQUOTE)
		nextc_dquote(lex);
	else
		nextc_normal(lex);
}
